...
skills from, and register `wiretrace-mcp` as an MCP server. After that the user
says "turn this portal into an API" and the agent knows the whole procedure —
which capture mode fits, how many samples inference needs, how to read a
dependency edge, and what a 401 actually means.
